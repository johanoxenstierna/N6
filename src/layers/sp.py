...
class Sp(AbstractLayer, AbstractSSS):
    ...
    def finish_info(_s):
        ...
        _s.gi['v'] = _s.gi['v'] - dist_to_straight_up * 150  # speed depends on angle
        _s.gi['v'] = max(30, _s.gi['v'])

        # if dist_to_straight_up < 0.1 and _s.gi['v'] > 120:  # frames tot updated elsewhere
        ...
        sp_len_stop = abs(int(np.random.normal(loc=_s.gi['sp_len_stop_loc'], scale=_s.gi['sp_len_stop_scale'])))
        sp_len_stop = max(8, sp_len_stop)

        _s.sp_lens = np.linspace(sp_len_start, sp_len_stop, num=_s.gi['frames_tot'], dtype=int)
